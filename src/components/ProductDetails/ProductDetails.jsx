...
export default function ProductDetails() {
  ...
  const filteredProducts = products?.filter((p) => {
    const matchCategory = p?.category === currentProduct?.category;

    const matchType = p?.subCategory === currentProduct?.subCategory;
  ...
  const relatedProducts = [...filteredProducts]
    ...
    ?.filter(
      (item, index, arr) =>
        index === arr.findIndex((p) => p.image[0] === item.image[0]),
    )
    .slice(0, 5);
  ...
  return (
    currentProduct && (
      <div className=" pt-10 mx-3 sm:mx-0">
        <div className="flex flex-col sm:flex-row gap-12">
          ...
          <div className="flex-1 flex flex-col-reverse  sm:flex-row gap-3">
            ...
            <div className="sm:w-[80%]">
              <img src={selectedImg} alt="" className="w-full h-full" />
            </div>
          </div>

          {/* Details */}
          <div className="flex-1">
            <h1 className="font-medium text-2xl mt-2">{currentProduct.name}</h1>
        ...
        <div className="mt-20">
          ...
          <div className="border border-secondary/20 dark:border-primary/80 px-6 py-6 text-sm text-gray-500 flex flex-col gap-4">
            ...
            <p>
              Each product usually has its own dedicated page with images,
              prices and details.
            </p>
          </div>
        </div>
